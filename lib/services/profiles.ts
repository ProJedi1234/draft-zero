// lib/services/profiles.ts — Model profile writes, callable from a server
// action, a route handler or an MCP tool alike.
//
// Profiles do not ride the bus as entities yet: every write announces a plain
// change through commitChange, so devices catch up by reading.
import "server-only"
...
import {
  ...
  saveStoryAsProfileInput,
  setDefaultProfileInput,
  setStoryProfileInput,
  updateProfileInput,
 ...
 * Parses here rather than in either caller, so saveStoryAsProfile judges the
 * name and the settings it copied after reading the story, as it always did.
 *
 * A profile feeds generation for every story that follows it, so an unknown
 * thinking level or an off-ladder context window would 400 (or render blank)
 * once per follower rather than once here.
 */
async function insertProfile(
  raw: CreateProfileInput
): Promise<ServiceResult<{ id: string }>> {
  const parsed = parseInput(createProfileInput, raw)
...
export const createProfile: Service<
  CreateProfileInput,
  { id: string }
> = async (raw) => {
  const created = await insertProfile(raw)
...
export const updateProfile: Service<UpdateProfileInput> = async (raw) => {
  const parsed = parseInput(updateProfileInput, raw)
  if (!parsed.ok) return parsed
  ...
  if (updated.length === 0) return fail("not_found", NOT_FOUND)
 ...
 */
export const reorderProfiles: Service<ReorderProfilesInput> = async (raw) => {
  const parsed = parseInput(reorderProfilesInput, raw)
  if (!parsed.ok) return parsed
  ...
  if (stale) return fail("conflict", "The profile list changed. Try again.")
...
export const setDefaultProfile: Service<SetDefaultProfileInput> = async (
  raw
) => {
  const parsed = parseInput(setDefaultProfileInput, raw)
  if (!parsed.ok) return parsed
  ...
  const profile = await db
    ...
    .limit(1)
    .then((rows) => rows[0])
  if (!profile) return fail("not_found", NOT_FOUND)
 ...
 */
export const deleteProfile: Service<DeleteProfileInput> = async (raw) => {
  const parsed = parseInput(deleteProfileInput, raw)
  if (!parsed.ok) return parsed
  const { id } = parsed.data

  const settings = await getAppSettings()
  if (settings.defaultProfileId === id) {
    return fail("conflict", "Make another profile the default first.")
  }
  ...
  if (!deleted) return fail("not_found", NOT_FOUND)
 ...
 * where they left off.
 */
export const setStoryProfile: Service<SetStoryProfileInput> = async (raw) => {
  const parsed = parseInput(setStoryProfileInput, raw)
  if (!parsed.ok) return parsed
  const { storyId, profileId } = parsed.data
  ...
  if (updated.length === 0) return fail("not_found", "Story not found.")
 ...
 */
export const saveStoryAsProfile: Service<
  SaveStoryAsProfileInput,
  { id: string }
> = async (raw) => {
  const parsed = parseInput(saveStoryAsProfileInput, raw)
  if (!parsed.ok) return parsed
