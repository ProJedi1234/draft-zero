// lib/services/import.ts — The importers, callable from a server action, a
// route handler or an MCP tool alike. Each one writes a whole story (or a
// lorebook's worth of rows) in one transaction, then commits it to the library.
//
// Every service takes a ServiceContext for parity with the rest of lib/services,
// but none reads it: an import publishes an uncovered `change` event, which
// carries no origin, so every device — the importer included — catches up.
import "server-only"
...
import { fillScenarioPlaceholders, parseScenario } from "@/lib/import/novelai"
...
import {
  importBackupInput,
  importScenarioInput,
  importStoryCardsInput,
  importStoryCardsIntoStoryInput,
  ...
  type StoryCardImportSummary,
  type StoryCardMergeSummary,
 ...
 * gets written is re-derived here — the service trusts the file text and the
 * placeholder values, nothing else.
 */
export const importScenario: Service<
  ImportScenarioInput,
  ScenarioImportSummary
> = async (raw) => {
  const input = parseInput(importScenarioInput, raw)
  if (!input.ok) return input

  const parsed = parseScenario(input.data.json)
  if (!parsed.ok) return fail("invalid", parsed.error)

  const scenario = fillScenarioPlaceholders(
    parsed.data,
    input.data.placeholderValues
  )
 ...
 *
 * The client parses the same bytes to render its preview, but the payload that
 * gets written is re-derived here — the service trusts the file text, nothing
 * else.
 */
export const importStoryCards: Service<
  ImportStoryCardsInput,
  StoryCardImportSummary
> = async (raw) => {
  const input = parseInput(importStoryCardsInput, raw)
  if (!input.ok) return input

  const parsed = parseStoryCards(input.data.json)
  if (!parsed.ok) return fail("invalid", parsed.error)
 ...
 * all in one write.
 *
 * The archive arrives as a `File` and not as text, unlike the two JSON
 ...
 */
export const importAiDungeonBackup: Service<
  ImportBackupInput,
  BackupImportSummary
> = async (raw) => {
  const input = parseInput(importBackupInput, raw)
  if (!input.ok) return input

  const parsed = await parseBackup(await input.data.file.arrayBuffer())
  if (!parsed.ok) return fail("invalid", parsed.error)
 ...
 */
export const importStoryCardsIntoStory: Service<
  ImportStoryCardsIntoStoryInput,
  StoryCardMergeSummary
> = async (raw) => {
  const input = parseInput(importStoryCardsIntoStoryInput, raw)
  if (!input.ok) return input
  const { storyId } = input.data
  ...
  if (missing) return fail("not_found", "Story not found.")
  ...
  commitChange(storyId, ["lorebook-entry"])
  return ok({
    storyId,
