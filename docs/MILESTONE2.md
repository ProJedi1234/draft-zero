...
`components/ui/{input,textarea}.tsx` pin `defaultValue` to its mount value for you (`useInitialValue`), so passing a live server prop — `defaultValue={story.title}`, which changes on every revalidation — is safe and silent. Without the pin, Base UI's `FieldControl` warns *"A component is changing the default value state of an uncontrolled FieldControl after being initialized"*; the value never actually moved, only the prop.

**The magic-sync exception.** "Never resync from props" predates the change bus, and taken literally it is what makes a setting changed on one device invisible on another: the `change` event lands, the tree refetches, and a `useState(prop)` seeded at mount ignores the result. A control that must follow the server while mounted uses `hooks/use-server-synced.ts` — `useServerSyncedField` for text (reconciles imperatively into the DOM; never switch to `value`), `useServerSyncedValue` for comboboxes, selects and sliders. Both adopt an incoming server value only when the control has no write of its own in flight and is not being edited: `useServerSyncedField` checks focus itself, while `useServerSyncedValue` knows only what its caller passes as `hold` — the inspector holds its sliders for the duration of a drag (`hooks/use-drag-hold.ts`) and its three pickers while any of their menus is open. "No write in flight" is released by the echo of that write *or* by the caller reporting how the save ended (`settle()` / `reset()`), which every call site must do: a save that fails, or a write from another device that wins the row, produces no echo at all, and a control still waiting for one has silently stopped following the server. Applies to: the inspector's model / provider / thinking / context window and all five numeric sliders, and the settings page's default model + thinking.
