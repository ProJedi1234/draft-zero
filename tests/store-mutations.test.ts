...
// the actions this file's subject imports carry the marker on purpose.
// Neutralising it is what lets story-mutations be loaded here at all — and it
// is a leaf, so unlike a module mock on the actions themselves it poisons
// nothing for any other file. See the same note in tests/generation-stream.
...
/**
 * Injected through story-mutations' own seam rather than with mock.module.
 *
 * Bun's mock.module replaces the module for the whole test PROCESS, not just
 * this file — so mocking "@/lib/actions/stories" here handed a stub
 * `deleteStory` to every test file loaded afterwards. tests/generation-stream
 * calls the real one to prove a delete aborts the story's live run, and with
 * the stub in its place that run never ended and the test sat out its timeout.
 * It survived locally purely because the file order happened to be kind, and
 * failed in CI where it was not.
 */
const mockedActions = {
...
beforeEach(() => {
  ...
  mutationQueue.reset()
  clientStore.reset()
  mutations.setStoryMutationDepsForTests(mockedActions)
})

afterEach(() => {
  // Leave the real actions in place for whatever runs next.
